/* Queens (1)
 * Write a program to compute the number of ways to place n queens on an nxn 
 *  chessboard so that no queen threates another queen. That is, no two queens
 *  can be located on the same row, column or diagonal.
...
int queens(int n, int i, std::vector<int>& t, std::vector<bool>& mc, 
           std::vector<bool>& mda, std::vector<bool>& mdd)
{
  if (i == n) return 1; // solution found (n queens in nxn chessboard ...)
  else {
    int c = 0;
    for (int j = 0; j < n; ++j) {
