/* Queens (3)
 * Write a program to print the lexicographically smallest way to place n queens
 *  on an nxn chessboard so that no queen threatens another queen.
 * Input: natural number n > 0
 * Output: lexicographically smallest way ...where queens are marked with a 'Q'
 *  and empty positons with a dot.
*/
...
bool queens(int n, int i, std::vector<int>& t, std::vector<bool>& mc, 
            std::vector<bool>& mda, std::vector<bool>& mdd)
{
  if (i == n) {
    print(n, t); // solution found (n queens in nxn chessboard ...)
    return true;
  }
  else {
    for (int j = 0; j < n; ++j) {
      if (not mc[j] and not mdd[i+j] and not mda[i-j+n-1]) {
        ...
        if (queens(n, i+1, t, mc, mda, mdd)) return true;
...
bool queens(int n)
{
  ...
  return queens(n, 0, t, mc, mda, mdd);
}
...
int main()
{
  ...
  if (not queens(n)) std::cout << "NO SOLUTION" << std::endl;
}
