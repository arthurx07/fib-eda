 ...
 * and two vertices x and y, computes the minimum cost to go from x to y, and the
 * minimum number of steps of all the paths that go from x to y with such minimum cost.
 *
...
std::pair<int, int> dijkstra(graph& G, int x, int y)
{
  ...
  return std::make_pair(dist[y], steps[y]);
}
...
int main()
{
  ...
  while (std::cin >> n >> m)
  {
    ...
    if (not (cs.first == -1 and cs.second == -1)) {
      std::cout << "cost " << cs.first << ", " << cs.second << " step(s)" << std::endl;
    } else
