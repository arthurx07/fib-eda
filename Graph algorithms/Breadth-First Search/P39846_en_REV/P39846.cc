 ...
 * distance from a given initial position to the furthest accessible treasure.
 * The allowed movements are horizontal or vertical, but not diagonal. If needed,
...
int bfs(const map& M, int r, int c, int n, int m)
{
  ...
  if (not tdist.empty()) return tdist.top();
  return -1;
}
...
int main()
{
  ...
  int min;
  if ((min = bfs(M, r-1, c-1, n, m)) > 0) std::cout << "maximum distance: " << min << std::endl;
